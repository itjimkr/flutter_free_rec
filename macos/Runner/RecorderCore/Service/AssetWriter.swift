...
final class AssetWriter: CaptureEngineSampleBufferDelegate, @unchecked Sendable {
    ...
    private var pauseStartSourceTime: CMTime = .invalid
    ...
    func resumeWriting() {
        lock.withLockUnchecked {
            ...
            isPaused = false
            logger.info("AssetWriter resumed")
        }
    }
    ...
    func appendAudioSample(_ sampleBuffer: CMSampleBuffer) {
        lock.withLockUnchecked {
            ...
            if lastSystemAudioPresentationTime.isValid
                && presentationTime <= lastSystemAudioPresentationTime
            {
                return
            }

            guard
                let adjustedSampleBuffer = retimedSampleBuffer(
                    sampleBuffer, newPresentationTime: presentationTime)
            else {
                return
            }
            ...
            if !audioInput.append(adjustedSampleBuffer) {
                ...
                if systemAudioSampleCount == 1 {
                    logger.info("First system audio sample appended successfully")
                    NSLog("%@", "[Rec][Audio] first system audio sample appended")
                }
            }
        }
    }
    ...
    func appendMicrophoneSample(_ sampleBuffer: CMSampleBuffer) {
        lock.withLockUnchecked {
            ...
            if lastMicrophonePresentationTime.isValid
                && presentationTime <= lastMicrophonePresentationTime
            {
                return
            }
            ...
            if !microphoneInput.append(adjustedSampleBuffer) {
                ...
                if microphoneSampleCount == 1 {
                    logger.info("First microphone sample appended successfully")
                    NSLog("%@", "[Rec][Audio] first microphone sample appended")
                }
            }
        }
    }
    ...
    func finishWriting() async throws -> URL {
        ...
        do {
            (writerToFinish, url) = try lock.withLockUnchecked {
                ...
                logger.info(
                    "Finishing writing - status: \(assetWriter.status.rawValue), session started: \(self.hasStartedSession), frames written: \(self.frameCount), system audio samples: \(self.systemAudioSampleCount), microphone samples: \(self.microphoneSampleCount)"
                )
        ...
        return try lock.withLockUnchecked {
            ...
            logger.info(
                "AssetWriter finished writing \(self.frameCount) frames, \(self.systemAudioSampleCount) system audio samples, \(self.microphoneSampleCount) microphone samples to: \(url.lastPathComponent)"
            )
            NSLog(
                "%@",
                "[Rec][Audio] finished frames=\(self.frameCount) systemAudioSamples=\(self.systemAudioSampleCount) microphoneSamples=\(self.microphoneSampleCount)"
            )
            frameCount = 0
    ...
    private func adjustedPresentationTime(for sourcePresentationTime: CMTime) -> CMTime? {
        if isPaused {
            if !pauseStartSourceTime.isValid {
                pauseStartSourceTime = sourcePresentationTime
            }
            return nil
        }

        if pauseStartSourceTime.isValid {
            let pausedDuration = CMTimeSubtract(sourcePresentationTime, pauseStartSourceTime)
        ...
        if accumulatedPauseDuration.isValid
            && CMTimeCompare(accumulatedPauseDuration, .zero) > 0
        {
            return CMTimeSubtract(sourcePresentationTime, accumulatedPauseDuration)
        }

        return sourcePresentationTime
    }

    private func retimedSampleBuffer(
        _ sampleBuffer: CMSampleBuffer, newPresentationTime: CMTime
    ) -> CMSampleBuffer? {
        ...
        guard countStatus == noErr else {
            logger.error("Failed to inspect sample timing info")
            return nil
        }
        ...
        guard timingStatus == noErr, let firstTiming = timingInfo.first else {
            logger.error("Failed to load sample timing info")
            return nil
        }

        let offset = CMTimeSubtract(firstTiming.presentationTimeStamp, newPresentationTime)
        if !offset.isValid || CMTimeCompare(offset, .zero) == 0 {
            return sampleBuffer
        }
        ...
        guard copyStatus == noErr else {
            logger.error("Failed to retime sample buffer")
            return nil
        }
